...
function getUsers() {
    ...
    axios.get(`${mainURL}/users/${id}`)
        .then((response) => {
            ...
            document.getElementById("posts-profile").innerHTML = user.posts_count
            document.getElementById("comments-profile").innerHTML = user.comments_count

            document.querySelector("#username-posts span").innerHTML = user.username
        })
}
getUsers()
...
function getPosts() {
    ...
    axios.get(`${mainURL}/users/${id}/posts`)
        .then((response) => {
            toggleLoader(false)
